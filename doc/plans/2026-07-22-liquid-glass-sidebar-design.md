...
**Status:** Owner-approved design; square-shell revision approved 2026-07-23
...
Paperclip's primary navigation should feel like a permanent operational rail made from liquid glass. It remains dense, fast, and recognizably Paperclip, but loses both the flat application-panel treatment and the rounded floating-card silhouette. The visual language is dark-first liquid glass: restrained refraction, a fine edge highlight, background depth, and neutral diffuse light for interaction states.
...
### 1. One square glass rail

The primary `SidebarShell` panel remains the material boundary, but its desktop silhouette returns to the old fork's square rail. It is flush to the viewport's top, left, and bottom edges, has no outer corner radius, and uses one restrained right-hand divider instead of an all-around floating-card border. The `Sidebar` inside stays transparent so the rail and expanded navigation read as one continuous glass surface rather than nested panes.

Collapsed, expanded, and peeked states are the same square rail at different widths. The glass fill, blur, saturation, background refraction, and neutral diffuse navigation states remain unchanged. Peeking may strengthen the right-edge shadow slightly because it overlaps work, but must not introduce rounded corners, viewport gaps, or a floating-card outline.

Mobile is intentionally unchanged in this revision. It remains the existing off-canvas drawer; the square, flush silhouette applies only to the desktop primary sidebar.
...
- The workspace uses the recovered old-fork background library, with the current company selection resolved behind the shell through semantic tokens.
...
- Background image, position, and overlay remain token-driven so the existing Appearance gallery can switch scenes without rewriting shell components.

The background gallery, uploads, and company-level persistence are already implemented and remain unchanged by this sidebar-silhouette revision.
...
- desktop rail edge geometry and the existing mobile instrument inset/radius values.
...
- `ui/src/components/SidebarShell.tsx`: owns the square desktop glass rail across expanded, collapsed, and peek states while preserving the mobile drawer.
...
- `ui/public/backgrounds/`: contains the recovered local preset library.
...
4. Real-browser desktop proof covers the flush square silhouette plus expanded, collapsed rail, hover peek, resizing, active route, hover, keyboard focus, pressed state, and a long-scroll page.
...
- Any mobile drawer silhouette change
